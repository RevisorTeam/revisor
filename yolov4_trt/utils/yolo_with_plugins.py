...
try:
    ...
    ctypes.cdll.LoadLibrary('yolov4_trt/plugins/libyolo_layer.so')
except OSError as e:
    raise SystemExit('ERROR: failed to load yolov4_trt/plugins/libyolo_layer.so.  '
...
class TrtYOLO(object):
    ...
    def _load_engine(self):
        model_filename = '{}.trt'.format(self.model)
        TRTbin = os.path.join(self.model_dir, model_filename)
        with open(TRTbin, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(f.read())

    def __init__(self, model, input_shape, category_num=80, letter_box=False,
                 cuda_ctx=None, model_dir='yolo', gpu_id=0):
